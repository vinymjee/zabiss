...
# 1. Backend PHP SQLite sur 8080
Write-Host "[1/2] Backend http://127.0.0.1:8080/api/health (SQLite)" -ForegroundColor Green
...
try { Invoke-RestMethod http://127.0.0.1:8080/api/health -TimeoutSec 5 | Out-Null; Write-Host "  -> Backend OK" -ForegroundColor Green } catch { Write-Host "  -> Backend erreur: $_" -ForegroundColor Red; Receive-Job $backendJob -Keep | Select-Object -Last 20 }

# 2. Frontend Angular sur 4200
Write-Host "[2/2] Frontend http://localhost:4200 (proxy -> 8080)" -ForegroundColor Green
